...
void do_where( CHAR_DATA *ch, char *argument )
{
	...
	if (arg[0] == '\0')
	{
		...
		for (fpl = mud->f_player ; fpl ; fpl = fpl->next)
		{
			...
			if (findpath_search_victim(ch, victim, 12) == -1)
			{
				continue;
			}
	...
	victim = findpath_search_name(ch, arg, 12);
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	int sn, class, cnt, cost, practice, adept;
	...
	else
	{
		...
		if (ch->pcdata->mclass[class] < skill_table[sn].skill_level[class])
		{
			send_to_char( "You can't practice that.\n\r", ch );
		...
		if (mob->pIndexData->level < skill_table[sn].skill_level[class])
		{
		...
		if (ch->pcdata->learned[sn] >= class_table[class].skill_master)
		{
		...
		cost = skill_table[sn].skill_level[class] * skill_table[sn].skill_level[class] * 5;
		...
		if (ch->pcdata->learned[sn] >= class_table[class].skill_adept)
		{
		...
		else
		{
			...
			{
				ch->pcdata->learned[sn] = class_table[class].skill_adept;
...
void do_class( CHAR_DATA *ch, char *arg)
{
	...
	if (buf[0] == '\0')
	{
		...
		for (cnt = 0 ; cnt < MAX_CLASS ; cnt++)
		{
			if (ch->pcdata->mclass[cnt] == 0 && race_table[ch->race].race_class[cnt] == 1)
			{
				strcat(buf," ");
	...
	for (cnt = 0 ; cnt < MAX_CLASS ; cnt++)
	{
		...
		&&  race_table[ch->race].race_class[cnt] == 1)
		{
