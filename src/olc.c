...
bool del_extra(EXTRA_DESCR_DATA *head, EXTRA_DESCR_DATA *tail, char *keywords )
{
	EXTRA_DESCR_DATA *ed;
...
void do_medit( CHAR_DATA *ch, char *argument )
{
	...
	if (!strcasecmp(arg2, "rmed"))
	{
		...
		if (!del_extra(mob->first_extradesc, mob->last_extradesc, argument))
		{
...
void do_oedit( CHAR_DATA *ch, char *argument )
{
	...
	if (!strcasecmp(arg, "rmed"))
	{
		...
		if (!del_extra(obj->first_extradesc, obj->last_extradesc, argument))
		{
			ch_printf(ch, "Keyword: %s not found.\n\r", argument);
		}
		pop_call();
...
void do_redit( CHAR_DATA *ch, char *argument )
{
	...
	if (!strcasecmp(arg, "rmed"))
	{
		...
		if (!del_extra(room->first_extradesc, room->last_extradesc, argument))
		{
			ch_printf(ch, "Keyword: %s not found.\n\r", argument);
		}
		pop_call();
