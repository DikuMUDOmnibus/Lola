...
void write_to_buffer(DESCRIPTOR_DATA *d, char *txt, int length)
{
	...
	if (ch)
	{
		if (HAS_BIT(ch->pcdata->vt100_flags, VT100_INTERFACE) && d->connected >= CON_PLAYING && length != 1000000)
		{
			...
			if (d->outtop == 0 && IS_SET(d->comm_flags, COMM_FLAG_COMMAND))
			{
				strcat(buf, "\033[0K");
			}
		...
		{
			if (!HAS_BIT(ch->pcdata->vt100_flags, VT100_INTERFACE) && d->connected >= CON_PLAYING && d->outtop == 0 && !IS_SET(d->comm_flags, COMM_FLAG_COMMAND))
			{
				d->outbuf[0] = '\n';
				d->outbuf[1] = '\r';
				d->outbuf[2] = '\0';
